!      Usage ifort -o difffinger.x   pearson.o  difffinger.f90
!      ./difffinger.x  POSCAR_1 POSCAR_2
!234567890
...
!      Written by In-Ho Lee, KRISS, October 2, 2020.
       ...
       integer i,j
       real*8 tmp
       character*280 fname,gname,fname1,gname1
       integer ninv0,nsigma0,nrref0
       integer ndata
       real*8, allocatable :: vorkq(:,:)
       logical lexist
       ...
       STOP
       ENDIF
       CALL GET_COMMAND_ARGUMENT(1,fname)   !first, read in the two values
       CALL GET_COMMAND_ARGUMENT(2,fname1)
!      READ(num1char,*) num1                    !then, convert them to
!      READ(num2char,*) num2
       ninv0=9 ; nsigma0=1 ; nrref0=200
       ndata=ninv0*nsigma0*nrref0
       allocate(vorkq(ndata,2))
       fname=adjustl(fname) ; fname=trim(fname)
       gname=trim(fname)//'_fin' ; gname=trim(gname)
       write(6,*) trim(fname),' ', trim(gname)
       call invfeature(fname,gname,ninv0,nsigma0,nrref0)
       fname1=adjustl(fname1) ; fname1=trim(fname1)
       gname1=trim(fname1)//'_fin' ; gname1=trim(gname1)
       write(6,*) trim(fname1),' ', trim(gname1)
       call invfeature(fname1,gname1,ninv0,nsigma0,nrref0)
       open(17,file=trim(gname),form='formatted')
       read(17,*)
       do j=1,ndata
       read(17,*) vorkq(j,1)
       enddo
       close(17)
       open(18,file=trim(gname1),form='formatted')
       read(18,*)
       do j=1,ndata
       read(18,*) vorkq(j,2)
       enddo
       close(18)
       call pearson(vorkq(1,1),vorkq(1,2),ndata,tmp)
       write(6,*) tmp,trim(fname),' ',trim(fname1)
       deallocate(vorkq)
