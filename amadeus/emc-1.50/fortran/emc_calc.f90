...
! { added by In-Ho Lee, KRISS, April 18, 2016.
if(prg .eq. 'V') then
    dk = dk/(2.0D0*pi*b2a)
...
if (ok .eq. 0) then
    write(ilog,*) "Principal effective masses and directions:"
    do i=1,3
        write(ilog,"(A25,F12.4)") "Effective mass:", 1.0D0/b(i)
        write(ilog,"(A25,3F12.6)") "Cartesian coordinate:", (m(j,i), j=1,3)
        v = pureDGESV(f, m(:,i), 'T')
        v = normal(v, size(v))
        write(ilog,"(A25,3F12.6)") "Direct lattice vectors:", (v(j), j=1,3)
        write(ilog,*)
    end do
else
