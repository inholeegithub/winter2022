!234567890
!      Written by In-Ho Lee, KRISS, April 13, 2014.
       subroutine softmutation(ndeg,qqq,jjcc,amp,t6)
       USE csa_application, ONLY : nspecies,sigmamatrix
       implicit none
       integer ndeg,jjcc
       real*8 qqq(ndeg),amp,t6(6)
       real*8 a1(3),a2(3),a3(3),r6(6),cmatrix(3,3)
       real*8 tmp,tmq,tms,de,rcut,bdmp,tmqi,tmqj
       integer ish,i,j,na,na3,i3,j3,isoft,jsoft,maxnn
       integer n,nm,ierr,jerr,matz
       real*8, allocatable :: rnei(:,:),aforce(:,:),bforce(:,:),force(:,:),drt1(:,:),drt2(:,:),drt(:,:)
       integer, allocatable :: nnei(:,:),numn(:)
       real, allocatable :: xx(:,:),w(:),z(:,:),fv1(:),fv2(:)
       real ranmar
       logical ltrivial

       ltrivial=.false.
       bdmp=0.0d0 ; rcut=0.d0
       do i=1,nspecies
       do j=1,nspecies
       rcut=rcut+sigmamatrix(i,j)
       bdmp=bdmp+1.d0
       enddo
       enddo
       rcut=rcut/bdmp
...
!      print*, bdmp,rcut
       ...
       drt(i,3)=qqq(3*(i-1)+3)
       enddo
       allocate(drt1(na,3)) ; allocate(drt2(na,3))
       allocate(force(na,3)) ; allocate(bforce(na,3)) ; allocate(aforce(na,3)) 
       allocate(nnei(maxnn,na),numn(na)) ; allocate(rnei(maxnn,na))
       call softnnegh(na,drt,numn,nnei,rnei,a1,a2,a3,rcut,maxnn)
       jerr=0
       do i=1,na
       if(numn(i) < 1) jerr=jerr+1
       enddo
       na3=na*3
       allocate(xx(na3,na3))
       call tirionforce(na,drt,bforce,a1,a2,a3,rcut,numn,nnei,rnei,maxnn)
       xx=0.0
       do i=1,na
       do i3=1,3

       drt1=drt
       call tocar(drt1,na,a1,a2,a3)
       drt1(i,i3)=drt1(i,i3)+de
       drt2=drt1
       call tolat(drt2,na,a1,a2,a3)
       call tirionforce(na,drt2,force,a1,a2,a3,rcut,numn,nnei,rnei,maxnn)
       drt1=drt
       call tocar(drt1,na,a1,a2,a3)
       drt1(i,i3)=drt1(i,i3)-de
       drt2=drt1
       call tolat(drt2,na,a1,a2,a3)
       call tirionforce(na,drt2,aforce,a1,a2,a3,rcut,numn,nnei,rnei,maxnn)

       do j=1,na
       do j3=1,3
       tmp=-( force(j,j3)-bforce(i,i3))/de
       tmq= (aforce(j,j3)-bforce(i,i3))/de
       xx(3*(i-1)+i3,3*(j-1)+j3)=xx(3*(i-1)+i3,3*(j-1)+j3)+sngl(tmp+tmq)/2.0
       enddo
       enddo
       enddo
       enddo
       n=na3 ; nm=n ; matz=n
       allocate(w(n),z(nm,n),fv1(n),fv2(n))
       w=0.
       z=0.
       if(na <=4)then
       ltrivial=.true.
       ...
       ierr=0
       if(jerr == 0) call fake_rs(nm,n,xx,w,matz,z,fv1,fv2,ierr)
       if(jerr > 1) ierr=1
       if(ierr /= 0)then
       ltrivial=.true.
       goto 911
 ...
 100   continue
       isoft=6+(1+dble(ranmar())*6)
       jsoft=6+(1+dble(ranmar())*6)
       if(isoft == jsoft) goto 100
       if(isoft >= na3)then
       isoft=(1+dble(ranmar())*na3)
       ltrivial=.true.
                       endif
       if(jsoft >= na3)then
       jsoft=(1+dble(ranmar())*na3)
       ltrivial=.true.
                       endif
       if(.not. ltrivial)then
       drt1=drt
       call tocar(drt1,na,a1,a2,a3)
       tmp=bdmp*(ranmar()/4.d0+1.5d0) ; if(ranmar() > 1.5) tmp=-tmp
!
       tmqi=-1.d0
       tmqj=-1.d0
       do i=1,na
       do i3=1,3
       if(tmqi < abs(z(isoft,3*(i-1)+i3))) tmqi=abs(z(isoft,3*(i-1)+i3))
       if(tmqj < abs(z(jsoft,3*(i-1)+i3))) tmqj=abs(z(jsoft,3*(i-1)+i3))
       enddo
       enddo
!
       tms=tmp/tmqi
       write(6,'(3f16.5,2x,a12)') tmp,tmqi,tms,'tmp,tmqi,tms'
       do i=1,na
       do i3=1,3
       drt(i,i3)=drt1(i,i3)+tms*z(isoft,3*(i-1)+i3)+(ranmar()-0.5)*0.2
       enddo
       enddo
       if(na3 >= 12)then
       write(6,'(a4,1x,2i6,6e11.3)') 'soft',jjcc,isoft,(w(i),i=7,min(12,na3))
                    endif
       if(ranmar() < 0.2)then
       tmp=bdmp*(ranmar()/4.d0+1.5d0) ; if(ranmar() > 1.5) tmp=-tmp
       tms=tmp/tmqj
       write(6,'(3f16.5,2x,a12)') tmp,tmqj,tms,'tmp,tmqj,tms'
       do i=1,na
       do i3=1,3
       drt(i,i3)=drt(i,i3)+tms*z(jsoft,3*(i-1)+i3)+(ranmar()-0.5)*0.2
       enddo
       enddo
       write(6,'(a4,1x,2i6)') 'soft',jjcc,jsoft
                         endif
                         endif
 911   continue
       if(ltrivial)then
       tmp=bdmp*(ranmar()/4.d0+1.5d0)
       drt1=drt2
       do i=1,na
       do i3=1,3
       drt(i,i3)=drt1(i,i3)+tmp*(ranmar()-0.5)
       enddo
       enddo
                   endif
!
...
!
       ...
       enddo
!
       deallocate(xx) ; deallocate(w,z,fv1,fv2)
       deallocate(nnei,numn) ; deallocate(rnei)
       deallocate(drt)
       deallocate(drt1) ; deallocate(drt2)
       deallocate(force) ; deallocate(bforce) ; deallocate(aforce)
       return
       end
!234567890
!      Written by In-Ho Lee, KRISS, April 13, 2014.
       subroutine tirionforce(na,drtx,force,a1,a2,a3,rcut,numn,nnei,rnei,maxnn)
       implicit none
       integer maxnn,na,numn(na),nnei(maxnn,na)
       real*8 rnei(maxnn,na),drtx(na,3),force(na,3)
       real*8 a1(3),a2(3),a3(3),rcut
       integer i,j,j0
       real*8 vec(3),wec(3),rr,fact,tmp
       ...
       wec(:)=drtx(i,:)-drtx(j,:)
       wec(1)=wec(1)-anint(wec(1))
       wec(2)=wec(2)-anint(wec(2))
       wec(3)=wec(3)-anint(wec(3))
       vec(1)=wec(1)*a1(1)+wec(2)*a2(1)+wec(3)*a3(1)
       vec(2)=wec(1)*a1(2)+wec(2)*a2(2)+wec(3)*a3(2)
       vec(3)=wec(1)*a1(3)+wec(2)*a2(3)+wec(3)*a3(3)
       rr=sqrt(dot_product(vec,vec))
!      fact=1.d0/(rr+1.d0)**2
       fact=1.d0
       tmp=rnei(j0,i)
       force(i,1)=force(i,1)-(rr-tmp)*vec(1)/rr*fact
       force(i,2)=force(i,2)-(rr-tmp)*vec(2)/rr*fact
       force(i,3)=force(i,3)-(rr-tmp)*vec(3)/rr*fact
                               endif
                      endif
       enddo 
       enddo 
...
!234567890
!      Written by In-Ho Lee, KRISS, April 13, 2014.
       subroutine softnnegh(na,drtx,numn,nnei,rnei,a1,a2,a3,rcut,maxnn)
       implicit none
       integer na,maxnn,nnei(maxnn,na),numn(na)
       real*8 rnei(maxnn,na),drtx(na,3),a1(3),a2(3),a3(3),rcut
       integer i,j,kk
       real*8 vec(3),wec(3),rr
...
!234567890
!      Written by In-Ho Lee, KRISS, April 13, 2014.
       subroutine softmutation1(ndeg,qqq,jjcc,amp)
       USE csa_application, ONLY : nspecies,sigmamatrix
       implicit none
       integer ndeg,jjcc
       real*8 qqq(ndeg),amp
...
!      print*, bdmp,rcut
       ...
       allocate(force(na,3)) ; allocate(bforce(na,3)) ; allocate(aforce(na,3)) 
       allocate(nnei(maxnn,na),numn(na)) ; allocate(rnei(maxnn,na))
       call softnnegh1(na,drt,numn,nnei,rnei,rcut,maxnn)
       jerr=0
       do i=1,na
       if(numn(i) < 1) jerr=jerr+1
       enddo
       na3=na*3
       allocate(xx(na3,na3))
       call tirionforce1(na,drt,bforce,rcut,numn,nnei,rnei,maxnn)
       xx=0.0
       do i=1,na
       do i3=1,3
       ...
       drt1(i,i3)=drt1(i,i3)+de
       drt2=drt1
       call tirionforce1(na,drt2,force,rcut,numn,nnei,rnei,maxnn)
       drt1=drt
       drt1(i,i3)=drt1(i,i3)-de
       drt2=drt1
       call tirionforce1(na,drt2,aforce,rcut,numn,nnei,rnei,maxnn)
...
!
       ...
       do i=1,na
       do i3=1,3
       drt(i,i3)=drt1(i,i3)+tms*z(isoft,3*(i-1)+i3)
       enddo
       enddo
       if(na3 >= 12)then
       write(6,'(a4,1x,2i6,6e11.3)') 'soft',jjcc,isoft,(w(i),i=7,min(12,na3))
       ...
       do i=1,na
       do i3=1,3
       drt(i,i3)=drt(i,i3)+tms*z(jsoft,3*(i-1)+i3)
       enddo
       enddo
       write(6,'(a4,1x,2i6)') 'soft',jjcc,jsoft
                         endif
...
!234567890
!      Written by In-Ho Lee, KRISS, April 13, 2014.
       subroutine tirionforce1(na,drtx,force,rcut,numn,nnei,rnei,maxnn)
       implicit none
       integer maxnn,na,numn(na),nnei(maxnn,na)
       real*8 rnei(maxnn,na),drtx(na,3),force(na,3)
       real*8 rcut
       integer i,j,j0
       real*8 vec(3),rr,fact,tmp
...
!234567890
!      Written by In-Ho Lee, KRISS, April 13, 2014.
       subroutine softnnegh1(na,drtx,numn,nnei,rnei,rcut,maxnn)
       implicit none
       integer na,maxnn,nnei(maxnn,na),numn(na)
       real*8 rnei(maxnn,na),drtx(na,3),rcut
       integer i,j,kk
       real*8 vec(3),rr
...
!234567890
!      Written by In-Ho Lee, KRISS, April 13, 2014.
       subroutine fake_rs(nm,n,xx,w,matz,z,fv1,fv2,ierr)
       implicit none
       integer nm,n,matz,ierr
       real xx(n,n),w(n),z(nm,n),fv1(n),fv2(n)
       real*8, allocatable :: qdr(:),qdi(:),tvecr(:),tveci(:),tey(:)
       real*8, allocatable :: wrk11(:),wrk13(:),wrk14(:),wrk15(:)
       real*8, allocatable :: wrk21(:),wrk22(:),wrk23(:),wrk24(:),wrk25(:)
       integer, allocatable :: iwrk31(:),iwrk32(:)
       integer kj,k,j,nbndum,ms,me
       real ranmar
       ...
       w(k)=1.d8
       do j=1,nm
       z(k,j)=(ranmar()-0.5)/float(nm)
       enddo
       enddo
       call gentvect(nm,z)
       return
                   endif
!      call rs(nm,n,xx,w,matz,z,fv1,fv2,ierr)
       ms=n
       me=n
       me=min(12,n)
       nbndum=n+5
       kj=(nbndum*(nbndum+1))/2
       allocate(qdr(kj),qdi(kj))
       allocate(iwrk31(nbndum),iwrk32(nbndum))
       allocate(wrk11(nbndum*2),wrk13(nbndum),wrk14(nbndum),wrk15(nbndum)) 
       allocate(wrk21(nbndum),wrk22(nbndum),wrk23(nbndum),wrk24(nbndum),wrk25(nbndum))
       allocate(tvecr(nbndum*nbndum),tveci(nbndum*nbndum),tey(nbndum))
       qdr=0.d0
       qdi=0.d0
       do 941 k=1,n
       do 941 j=1,k
       ...
       qdr(kj)=xx(k,j)
       qdi(kj)=0.d0
       if(j .eq. k) qdi(kj)=0.d0
  941  continue
!
       call diagc(tey,ms,me,qdr,qdi,tvecr,tveci, &
       iwrk31,iwrk32,wrk11,wrk13,wrk14,wrk15, &
       wrk21,wrk22,wrk23,wrk24,wrk25,nbndum)
       if(iwrk31(1) == -1)then
       do k=1,nm
       w(k)=1.d8
       do j=1,nm
...
!
       deallocate(iwrk31,iwrk32)
       deallocate(wrk11,wrk13,wrk14,wrk15)
       deallocate(wrk21,wrk22,wrk23,wrk24,wrk25)
       deallocate(qdr,qdi,tvecr,tveci,tey)
       return
       end
!234567890
!      Written by In-Ho Lee, KRISS, April 13, 2014.
       subroutine gentvect(n,z)
       implicit none
       integer n
       real z(n,n)
       real*8, allocatable :: vv(:,:)
       integer i,j,m,na0,mdim,iskip
       real ranmar
       ...
       if(ranmar() < 0.2)then
       na0=n/3
       mdim=3 ; iskip=100*(3*na0)+ranmar()*dble(10)
       call modessobol(mdim,na0,iskip,z)
                         endif
       return
       end
!234567890
!      Written by In-Ho Lee, KRISS, April 13, 2014.
       subroutine gramschmidt(n,m,vv)
       implicit none
       integer n,m
       real*8 vv(n,m)
       real*8 tmp,tmq
       ...
       tmp=0.d0
       do k=1,n
       tmp=tmp+vv(k,i)*vv(k,i)
       enddo
       if(tmp > 1.d-12) tmp=1.d0/sqrt(tmp)
       do k=1,n
       vv(k,i)=vv(k,i)*tmp
       enddo
       do j=i+1,m
       tmp=0.d0
       do k=1,n
       tmp=tmp+vv(k,j)*vv(k,i)
       enddo
       tmq=0.d0
       do k=1,n
       tmq=tmq+vv(k,i)*vv(k,i)
       enddo
       if(tmq > 1.d-12) tmq=1.d0/sqrt(tmq)
       tmp=tmp*tmq
       do k=1,n
       vv(k,j)=vv(k,j)-tmp*vv(k,i)
       enddo
       enddo
       enddo
...
!234567890
!      Written by In-Ho Lee, KRISS, November 27, 2017.
       subroutine modessobol(mdim,n,iskip,somodes)
       implicit none
       integer mdim,n,iskip
       real somodes(3*n,3*n)
       integer j,i,k,kase,ksgn,kth
       real*8 tmp,vec(3),wec(3)
       integer, allocatable :: iwrk(:)
       real*8, allocatable :: r(:,:),r0(:,:),wrk(:)
       real ranmar

       allocate(r(1:mdim,1:n),r0(1:mdim,1:n))
       allocate(iwrk(n)) ; allocate(wrk(n))
       do kth=1,3*n
       call i8_sobol_generate(mdim,n,iskip,r)
       if(mdim == 2)then
       vec(1)=sum(r(1,:))/n;  vec(2)= sum(r(2,:))/n
                    endif
       if(mdim == 3)then
       vec(1)=sum(r(1,:))/n;  vec(2)= sum(r(2,:))/n ; vec(3)=sum(r(3,:))/n
                    endif
       if(mdim == 3)then
       kase=1+ranmar()*dble(9)
       select case(kase)
       case(1)
       do j=1,n
       wrk(j)=r(1,j)
       ...
       r(:,j)=r0(:,iwrk(j))
       enddo
!      sites suffling, shifting
       j=0
       j=1
       if(j == 1)then
       r0=r
       kase=1+ranmar()*dble(n)
       j=0
       do k=kase,n
       j=j+1
       r(:,j)=r0(:,k)
...
!      make a node (zero, one, shifted zero)
       wec(1)=ranmar()-0.5d0
       wec(2)=ranmar()-0.5d0
       wec(3)=ranmar()-0.5d0
       if(mdim == 3)then
       ...
       r(3,:)=r(3,:)-vec(3)-wec(3)
       do j=1,n
       somodes(kth,3*(j-1)+1)=r(1,j)
       somodes(kth,3*(j-1)+2)=r(2,j)
       somodes(kth,3*(j-1)+3)=r(3,j)
       enddo
                    endif
       if(mdim == 2)then
       r(1,:)=r(1,:)-vec(1)-wec(1)
...
!      normalization
       ...
       tmp=0.d0
       do j=1,n
       tmp=tmp+somodes(kth,3*(j-1)+1)*somodes(kth,3*(j-1)+1) &
              +somodes(kth,3*(j-1)+2)*somodes(kth,3*(j-1)+2) &
              +somodes(kth,3*(j-1)+3)*somodes(kth,3*(j-1)+3)
       enddo
       tmp=sqrt(tmp) ; if(tmp < 1.d-8) tmp=1.d0
       tmp=1.d0/tmp
       do j=1,3*n
       somodes(kth,j)=somodes(kth,j)*tmp
       enddo
                    endif
       if(mdim == 2)then
       tmp=0.d0
       ...
       tmp=1.d0/tmp
       do j=1,n
       somodes(kth,3*(j-1)+1)=somodes(kth,3*(j-1)+1)*tmp
       somodes(kth,3*(j-1)+2)=somodes(kth,3*(j-1)+2)*tmp
       enddo
                    endif
                 endif
       enddo
...
!  Discussion:
!
!    This routine uses the integer precision corresponding to a KIND of
!    8.
!
!  Example:
!
...
!
!    Output, integer ( kind = 8 ) I8_BIT_LO0, the position of the low 1
!    bit.
!
  implicit none
...
!    to a KIND of 8.
!
!    Thanks to Francis Dalaudier for pointing out that the range of
!    allowed
...
!  Parameters:
!
!    Input, integer ( kind = 8 ) DIM_NUM, the number of spatial
!    dimensions.
...
!    sequence.
!
!    Output, real ( kind = 8 ) QUASI(DIM_NUM), the next quasirandom
!    vector.
!
  implicit none
...
!
    if ( dim_num < 1 .or. dim_max < dim_num ) then
      ...
      write ( *, '(a,i12)' ) '    2 <= DIM_NUM <= ', dim_max
      write ( *, '(a,i12)' ) '  But this input value is DIM_NUM = ', dim_num
      stop
    end if
...
!
!    Input, integer ( kind = 4 ) SKIP, the number of initial points to
!    skip.
!
...
!
!    Output, integer ( kind = 4 ) VALUE, the integer value read from the
!    string.
