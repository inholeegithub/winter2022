...
class crystal(object):
    ...
    def matrix2para(matrix):
        ...
        cell_para[5] = crystal.angle(matrix[0], matrix[1])
        cell_para[4] = crystal.angle(matrix[0], matrix[2])
        cell_para[3] = crystal.angle(matrix[1], matrix[2])
...
def read_multi_POSCAR(filename):
    strucs = []
    ...
    while True:
        ...
        if content == "":
            print('---------------------------------------------')
            print('finishing reading all structures')
            print(len(strucs), ' structures have been loaded')
            print('---------------------------------------------')
            break
        else:
            lattice_constant = float(f.readline().split()[0])
            ...
            tmp = crystal(fileformat='dict', lattice=lattice, 
                        atom_type=atom_type, composition=composition,
...
if __name__ == "__main__":
    ...
    parser.add_option("-n", "--number", dest="number",default=5, type='int',
                      help="the number of reflections from low to high, default 5", metavar="number")
    ...
    if options.structure.find('cif') > 0:
        ...
        fileformat = 'POSCAR'
        strucs = read_multi_POSCAR(filename=options.structure)
    ...
    for test in strucs:
        ...
        rank = xrd.pxrd[:, 5] > options.minimum_I
        pxrd = xrd.pxrd[rank, :]
        if len(rank) < options.number:
            N = len(xrd.pxrd)
        else:
            N = options.number
        theta1 = pxrd[:N,0]
        intensity1 = pxrd[:N,5]*100
        str1 = ["%5.2f" % member for member in theta1 ]
        str2 = ["%5.1f" % member for member in intensity1]
        print(str1, test.composition)
        theta.append(str1)
        intensity.append(str2)

    col_name = {'2theta(degree)': theta, \
