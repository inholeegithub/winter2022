...
!      Written by In-Ho Lee, KRISS, April 15, 2016.
       ...
       private
       save
       integer nr,ndim,nstrc,ndeg,natot
       ...
       public :: prdf_init,prdf_final,cmp_prdf,get_prdf,stepft
       public :: prdf0,prdftest,prdfsave,nstrc
       contains
!234567890
!      Written by In-Ho Lee, KRISS, April 15, 2016.
       subroutine prdf_init(rmax00,nspecies0,nelements0,symbl0,lpbc0)
       implicit none
       ...
       enddo
       if(rmax00 <= 0.d0) rmax00=10.d0
       rmax=rmax00
!      nr=1001 ; r0=0.0d0 ; r1=rmax*2.0d0 ; dr=(r1-r0)/dble(nr-1)
       if(rmax00 > 10.d0)then
       write(6,*) rmax00,' has been changed to',10.d0
       rmax00=10.d0
                         endif
...
!      gnuplot> splot "mat.dat" with pm3d
       program batch4cdiff
       USE prdf, ONLY : cmp_prdf,prdf_init,prdf_final,get_prdf
       USE prdf, ONLY : nstrc,prdf0,prdftest,prdfsave
       implicit none
       integer nspecies1,nspecies2,ndeg1,ndeg2,isize,npop
       integer i,j,kontrol
...
!      write(6,*) nstrc
       ...
       write(6,*) omega,tmp
       nspecies2=nspecies1 ; allocate(symbl2(nspecies2)) ; allocate(nelements2(nspecies2))
       nelements2=nelements1
       ndeg1=6+3*sum(nelements1) ; ndeg2=6+3*sum(nelements2)
       allocate(qqq1(ndeg1))
       isize=4
       do i=1,nstrc
       ...
       if(i == 1) call prdf_init(rmax00,nspecies1,nelements1,symbl1,lpbc0)
       call get_prdf(qqq1) ; prdfsave(:,:,i)=prdf0(:,:)
       enddo
       allocate(zmat(nstrc,nstrc)) ; zmat=0.d0
       do i=1,nstrc
       do j=1,nstrc
       if(j > i)then
       prdftest(:,:)=prdfsave(:,:,j)-prdfsave(:,:,i)
       call cmp_prdf(tmp)
       ...
       enddo
       close(11)
       deallocate(symbl1,symbl2) ; deallocate(nelements1,nelements2) ; deallocate(qqq1)
...
!      Written by In-Ho Lee, KRISS, April 13, 2016.
       ...
       sig=sig/xnorm
       sig=sqrt(sig)
       open(11,file='mat_hist.dat',form='formatted')
